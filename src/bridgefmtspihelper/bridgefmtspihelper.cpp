...
bool QBridgeFmtSpiHelper::ExecCommand(QIODevice *pIODevice, BRIDGE_CMDS cmd, QByteArray *pParamData)
{
    ...
    if(!pIODevice->isOpen()) {
        ...
        m_SendRawData.clear();
        /* cmd */
        m_SendRawData.append(bReadCmd ? static_cast<char>(cmd | 0x80) : static_cast<char>(cmd));
        /* cmd param */
        for(int iParam=0; iParam<BRIDGE_SPI_FRAME_LEN-1;iParam++) {
            if(pParamData && pParamData->size() > iParam) {
                ...
                m_SendRawData.append(static_cast<char>(0));
            }
        }
...
bool QBridgeFmtSpiHelper::PrepareWriteRam(QIODevice *pIODeviceCtl, const quint32 ui32Address)
{
    ...
    for(int iAddrByte=0; iAddrByte<BRIDGE_SPI_FRAME_LEN-1; iAddrByte++) {
        addrArr.append(static_cast<char>(ui32AddressWork >> 24));
...
bool QBridgeFmtSpiHelper::WriteRam(QIODevice *pIODeviceData, const TRam16Data &data)
{
    ...
    bool bOK = true;
    quint32 ui32CountTransfers = static_cast<quint32>(data.count());
    /* 16bit-word transfers */
    for(quint32 ui32Word=0; ui32Word<ui32CountTransfers; ui32Word++) {
        quint16 ui16Val = static_cast<quint16>(data[static_cast<int>(ui32Word)]);
        quint8 ui8Val = ui16Val/256;
        send16Block.append(static_cast<char>(ui8Val));
        ui8Val = ui16Val & 0xFF;
        send16Block.append(static_cast<char>(ui8Val));
...
bool QBridgeFmtSpiHelper::ReadRam(QIODevice *pIODeviceData, TRam16Data &data, const quint32 ui32WordCount)
{
    ...
    while(ui32WordsToRead > 0) {
        ...
        if(static_cast<quint32>(receive16Block.size()) != 2*ui32WordsInBlock) {
            bOK = false;
    ...
    if(bOK) {
        ...
        quint16 ui16Val;
        quint8 ui8Val;
        if(static_cast<quint32>(data.size()) != ui32WordCount) {
            data.resize(static_cast<int>(ui32WordCount));
        }
        for(quint32 ui32Word=0; ui32Word<ui32WordCount; ui32Word++) {
            // This looks odd but we have
            // * signed char on PC (QByteArray)
            // * endianess??
            ui8Val = static_cast<quint8>(m_ReceiveRawData[0 + 2*ui32Word]);
            ui16Val = 256 * ui8Val;
            ui8Val = static_cast<quint8>(m_ReceiveRawData[1 + 2*ui32Word]);
            ui16Val += ui8Val;
            data[static_cast<int>(ui32Word)] = static_cast<qint16>(ui16Val);
        }
    }
    return bOK;
}
